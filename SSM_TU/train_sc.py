...
model_yaml_path = 'SSM_TU/model_descriptions/S_Edge_tiny.yaml'
hyperparameters_yaml_path = 'SSM_TU/hyperparams/Speech_Commands_Default.yaml'
...
model_save_path = ExperimentManagerSaveFunction(path=results_path, config_dict=config_dict, saveFiles=[
                                                'src', 'train_sc.py', model_yaml_path, hyperparameters_yaml_path])
...
pd.DataFrame.from_dict(config_dict, orient='index').to_csv(
    os.path.join(model_save_path, 'config.csv'))
...
valid_loader = torch.utils.data.DataLoader(
    valid_set, batch_size=batch_size, shuffle=False, drop_last=False, num_workers=num_workers, pin_memory=pin_memory)
...
params_ssm_lr = [param for name, param in model.named_parameters(
) if 'B' in name or 'C' in name or 'Lambda' in name or 'log_step' in name]
params_other_lr = [param for name, param in model.named_parameters(
) if 'B' not in name and 'C' not in name and 'Lambda' not in name and 'log_step' not in name]

print("params_ssm_lr:", type(params_ssm_lr))
print("params_other_lr:", type(params_other_lr))
input("INPUT HAPPENED")
...
for epoch in range(epochs):
    train_loss, train_acc, sub_epoch_info = train_one_epoch(
        model, criterion, optimizer, train_loader, regularize=True, scheduler=scheduler, sub_epoch_documentation=10, augments_use=augments)
    ...
    if valid_loss < best_val_loss:
        ...
        torch.save(model.state_dict(), os.path.join(
            model_save_path, 'best_valid_loss_model.pt'))
    if val_acc >= best_val_acc:
        best_val_acc_epoch = epoch
        best_val_acc = val_acc
        torch.save(model.state_dict(), os.path.join(
            model_save_path, 'best_valid_acc_model.pt'))
    if epoch % 10 == 0:
        torch.save(model.state_dict(), os.path.join(
            model_save_path, f'epoch_{epoch}_model.pt'))
    ...
    df_sub_epoch = pd.concat([df_sub_epoch, new_row],
                             ignore_index=True).reset_index(drop=True)
    df_sub_epoch.to_csv(os.path.join(
        model_save_path, train_sub_epoch_history_csv))


print('Evaluate on test set')
model.load_state_dict(torch.load(os.path.join(
    model_save_path, f'best_valid_loss_model.pt'), weights_only=True))
